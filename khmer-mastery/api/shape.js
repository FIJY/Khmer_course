...
const FONT_FILENAME = 'KhmerOS_siemreap.ttf';
const FONT_PATHS = [
  path.resolve(__dirname, '..', 'public', 'fonts', FONT_FILENAME),
...
function resolveFontPath() {
  const found = FONT_PATHS.find((candidate) => fs.existsSync(candidate));
  if (!found) {
    throw new Error(`Font not found in: ${FONT_PATHS.join(', ')}`);
  }
  return found;
}
...
function ensureFonts() {
  ...
  fkFont = fontkit.openSync(fontPath);

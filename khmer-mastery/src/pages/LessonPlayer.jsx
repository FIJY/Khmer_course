...
export default function LessonPlayer() {
  ...
  return (
    ...
    >
      ...
      {/* --- ВИЗУАЛЬНЫЙ ДЕКОДЕР --- */}
      {type === 'visual_decoder' && (
        <div className="w-full bg-gray-900 border border-white/10 rounded-[2.5rem] p-6 shadow-[0_25px_60px_-40px_rgba(0,0,0,0.8)]">
          <div className="text-center space-y-2">
            <h2 className="text-lg font-black uppercase tracking-[0.3em] text-cyan-400">
              Visual Decoder
            </h2>
            <p className="text-sm text-gray-300">
              {current?.instruction || current?.hint || 'Task: tap the glyphs to reveal and identify them.'}
            </p>
          </div>

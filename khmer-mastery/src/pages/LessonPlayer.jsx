...
export default function LessonPlayer() {
  ...
  const fetchLessonData = async () => {
    try {
      setLoading(true);
      // 1. Получаем общую информацию об уроке (Заголовок 1.1, 1.2)
      ...
      // 2. Получаем все элементы урока (карточки, квизы)
  ...
  const playAudio = (audioFile) => {
    if (!audioFile) return;
    // Используем путь к папке public/sounds/
  ...
  const handleNext = async (quality = 3) => {
    ...
    // Если это карточка или квиз, обновляем SRS прогресс
    ...
    if (step < items.length - 1) {
      ...
      // Урок окончен — возвращаемся на карту
      navigate('/map');
    }
  };
    ...
    <div className="h-screen flex flex-col bg-black text-white font-sans overflow-hidden">
      ...
      <header className="p-4 flex justify-between items-center border-b border-white/5 bg-gray-900/20 backdrop-blur-md">
        ...
        <div className="text-center">
          {/* Отображаем заголовок урока (например, Lesson 1.1) */}
          ...
          >
            <div className={`relative h-[28rem] transition-all duration-500 [transform-style:preserve-3d] ${isFlipped ? '[transform:rotateY(180deg)]' : ''}`}>
              ...
              <div className="absolute inset-0 backface-hidden bg-gray-900 rounded-[3rem] border border-white/5 flex flex-col items-center justify-center p-10 text-center shadow-2xl">
                ...
                <p className="mt-8 text-gray-700 text-xs uppercase font-bold tracking-widest">Tap to flip</p>
              </div>
              ...
              <div className="absolute inset-0 backface-hidden [transform:rotateY(180deg)] bg-gray-900 rounded-[3rem] border-2 border-cyan-500/20 flex flex-col items-center justify-center p-10 text-center shadow-[0_0_40px_rgba(34,211,238,0.05)]">
                ...
                <h2 className="text-5xl font-black mb-4 tracking-normal text-white">
                  {current.back}
                </h2>
                {/* ТРАНСКРИПЦИЯ (Чтобы не пугать новичков) */}
                <p className="text-2xl text-cyan-400 font-bold italic mb-10 tracking-tight">
             ...
             <div className="space-y-3">
               {current.options.map((opt, i) => (
                 <button
                   ...
                   className="w-full p-6 bg-gray-900/50 border border-white/5 rounded-[2rem] text-left font-bold text-lg hover:border-cyan-500/50 hover:bg-gray-900 transition-all active:scale-[0.98] focus:outline-none focus:ring-2 ring-cyan-500/20"
                 >
                   {opt}
                 </button>
               ))}

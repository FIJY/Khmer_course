  ...
  return (
    <div className="h-[100dvh] flex flex-col bg-black text-white overflow-hidden font-sans">
      <header className="p-4 flex justify-between items-center border-b border-white/5 bg-gray-900/20">
        ...
        <div className="text-center">
          <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-cyan-500 mb-1">
            {lessonInfo?.title.split(':')[0] || `Lesson ${id}`}
          </h2>
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-8 relative">
          ...
          <div className="w-full max-w-sm" onClick={() => { setIsFlipped(!isFlipped); if(!isFlipped) playAudio(current.audio); }}>
            <div className={`relative h-96 transition-all duration-500 [transform-style:preserve-3d] ${isFlipped ? '[transform:rotateY(180deg)]' : ''}`}>
              ...
              <div className="absolute inset-0 backface-hidden [transform:rotateY(180deg)] bg-gray-900 rounded-[3rem] border-2 border-cyan-500/20 flex flex-col items-center justify-center p-10 text-center">
                ...
                <div className="p-6 bg-cyan-500 rounded-full text-black shadow-lg shadow-cyan-500/20 active:scale-90 transition-transform"><Volume2 size={32} /></div>
              </div>
            </div>
          </div>
        )}
        ...
        {type === 'quiz' && (
          <div className="w-full max-w-sm">
             <h2 className="text-2xl font-black mb-10 italic uppercase text-center tracking-tighter leading-tight">{current.question}</h2>
             <div className="space-y-3">
               {current.options.map((opt, i) => (
                 <button key={i} onClick={() => handleNext(opt === current.correct_answer ? 5 : 1)}
                   className="w-full p-6 bg-gray-900/50 border border-white/5 rounded-3xl text-left font-bold hover:border-cyan-500/50 transition-all active:scale-[0.98]">
                   {opt}
                 </button>
               ))}
             </div>
        ...
        {/* THEORY (ВОТ ОН, ВЕРНУЛСЯ!) */}
        {type === 'theory' && (
          <div className="w-full max-w-sm bg-gray-900 border border-white/10 p-10 rounded-[3rem] text-center shadow-2xl relative z-10 [transform:translateZ(0)]">
      ...
      <footer className="p-8">
        {type !== 'quiz' && (
          <button onClick={() => handleNext(3)} className="w-full py-5 bg-white text-black rounded-2xl font-black uppercase tracking-widest flex items-center justify-center gap-3 active:scale-95 transition-all">

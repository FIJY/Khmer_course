...
const BootcampSession = ({ onClose, practiceItems = [], title }) => {
  ...
  const slideTypeLabels = {
    title: 'Вступление',
    'no-spaces': 'No Spaces',
    'reading-algorithm': 'Алгоритм чтения',
    'meet-teams': 'Команды Sun/Moon',
    rule: 'Правило 80/20',
    ready: 'Готовность'
  };
  const slideSubtitle = phase === 'theory'
    ? `Экран: ${slideTypeLabels[currentSlide?.type] ?? currentSlide?.type ?? 'Теория'}`
    : 'Практика: Visual Decoder';

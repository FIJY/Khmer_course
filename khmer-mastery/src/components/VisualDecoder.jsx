...
export default function VisualDecoder({ data, onComplete, hideDefaultButton = false }) {
  ...
  const playAudio = (filename) => {
    if (!filename) return;
    const path = `/sounds/${filename}`; // Путь от корня public
    console.log("🔊 Playing:", path); // Смотри в консоль (F12)
  ...
  const handleCharClick = (char, index) => {
    ...
    // Пытаемся найти звук: конкретной буквы -> или целевой буквы
    const soundToPlay = char_audio_map?.[char] || char_audio_map?.[target_char];

    if (char === target_char) {
      setStatus('success');
      playAudio('success.mp3'); // Убедись, что этот файл есть
  ...
  return (
    <div className="w-full flex flex-col items-center justify-start min-h-[50vh] py-4">
      ...
      {/* 1. ГЛАВНОЕ СЛОВО (ЦЕЛЬНОЕ И КРАСИВОЕ) */}
      {/* Мы используем KhmerColoredText, чтобы буквы сливались правильно */}
      ...
      {/* 3. КНОПКИ РАЗБОРА (ЗДЕСЬ МЫ КЛИКАЕМ) */}
